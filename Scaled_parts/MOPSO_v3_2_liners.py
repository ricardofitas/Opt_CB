...
import time
import copy
import Optimization_CB_2_liners
...
def run_trial(trials):
    ...
    class PSO:
        def __init__(self, function, bounds, w, c1, c2, n_particles, n_iterations, funcoes_objetivo, folder, problem_ind): #AQUIIIIIIIIIIIIIIII
            ...
            for i in range(n_particles):
                ...
                obj2, obj3 = Optimization_CB_2_liners.critical_stress(particle.position)
                #obj4 = obj2
                obj1 = Optimization_CB_2_liners.weight(particle.position)
                #obj2 *= obj1

                obj5, obj4 = Optimization_CB_2_liners.calculate_ez(particle.position)

                if self.problem_ind == 1:
                    obj2 = obj4

                objectives = [obj1/obj5, obj1/obj2, obj3]

                self.history_all.append([i, particle.position, particle.velocity, objectives, particle.fitness, [obj1, obj2, obj5]])
        ...
        def search(self):
            for i in range(self.n_iterations):
                ...
                for k in range(n):          # CRIAÇÃO DAS NOVAS PARTICULAS
                    ...
                    # OBJECTIVES = [f1(vetor), f2(vetor), f3(vetor), ...]
                    obj2, obj3 = Optimization_CB_2_liners.critical_stress(position)
                    # obj4 = obj2
                    obj1 = Optimization_CB_2_liners.weight(position)
                    # obj2 *= obj1

                    obj5, obj4 = Optimization_CB_2_liners.calculate_ez(position)
                    ...
                    self.history_all.append([self.history_all[k][0], position, velocity, objectives, fitness,  [obj1, obj2, obj5]])
    ...
    def cc(point):
        xx = Optimization_CB_2_liners.critical_stress(point)
        return -1 if not xx[1] else 1
    ...
    def z2(position):
        obj1 = Optimization_CB_2_liners.weight(position)
        obj3, obj2 = Optimization_CB_2_liners.calculate_ez(position)
        xx = obj1/obj2
        return xx[0], obj1, obj2
    ...
    bounds = [[0.50, 0.52], [0.50, 0.52], [1, 50], [8, 50], [0.50, 0.52]] # [thickness liner; thickness flute; wavelength; height; thickness liner

    for problem_ind in range(2):
        print(problem_ind)

        pso = PSO(function=score, bounds=bounds, n_particles=50, n_iterations=200, w=0.7, c1=2.05, c2=2.05,
                  funcoes_objetivo=objetivos,
                  folder="prob_CB_3_v4_2_liners/EPSO" + str(problem_ind) + "/EPSO_" + str(trials),
