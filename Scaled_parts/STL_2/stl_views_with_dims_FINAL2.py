...
def detect_endwall_z_levels(poly: vtk.vtkPolyData, bounds, unit_scale,
                            *, x_tol_ratio=1e-4, z_cluster_tol=0.08, max_levels=8):
    """Detect distinct Z-levels from the end walls (points near x=xmin/xmax).

    This is used to draw internal horizontal lines in the End view (YZ) at the
    same heights as visible edges in the Side view.

    Parameters
    ----------
    z_cluster_tol : float
        Clustering tolerance in *scaled units* (mm). Increase a bit if your STL
        is noisy.
    """
    ...
    x_tol = max(1e-9, x_tol_ratio * xr)

    x = pts[:, 0]
    z = pts[:, 2] * unit_scale  # scaled

    mask = (np.abs(x - xmin) <= x_tol) | (np.abs(x - xmax) <= x_tol)
    zv = z[mask]
    if zv.size == 0:
        return None
    ...
    # Cluster near-equal z values
    levels = []
    cur = [float(zv[0])]
    for val in zv[1:]:
        if abs(float(val) - float(cur[-1])) <= z_cluster_tol:
    ...
    # Ensure unique / compact
    levels = sorted(levels)
    compact = []
    for lv in levels:
        if not compact or abs(lv - compact[-1]) > 0.5 * z_cluster_tol:
            compact.append(lv)

    if len(compact) > max_levels:
        compact = compact[:max_levels]
...
def outline_end_yz_clean(bounds, unit_scale, z_levels):
    """End view (YZ) clean outline + internal horizontal feature lines.

    We draw the outer bounding rectangle (ymin..ymax, zmin..zmax) and then add
    internal horizontal lines at any Z-levels detected from the end walls of
    the STL (points near x=xmin and x=xmax). This mirrors what is visible in
    the side view without drawing internal seam lines.

    Parameters
    ----------
    z_levels : list[float] | None
        Z levels in *scaled units* (e.g. mm) where a horizontal line should be drawn.
    """
    xmin, xmax, ymin, ymax, zmin, zmax = bounds
    y0 = ymin * unit_scale
    y1 = ymax * unit_scale
    ...
    if z_levels:
        # Avoid duplicating the outer frame lines
        tol = max(1e-6, 1e-3 * abs(z1 - z0))
        for zl in sorted(z_levels):
            if (zl > z0 + tol) and (zl < z1 - tol):
                polylines.append([(y0, zl), (y1, zl)])

    return polylines


def outline_side_xz_clean(poly: vtk.vtkPolyData, bounds, unit_scale,
                          bins=1400, smooth_w=7):
    """Side view (XZ) clean outline for corrugator-style parts.

    Goal: show the corrugation profile AND the base thickness, but avoid
    internal seam lines from repeated STL chunks.

    What we draw:
      1) Corrugation top envelope z_top(x)  (max Z per X-bin)
      2) Base bottom line at z_bottom (global min Z)
      3) Two end walls (verticals) from z_bottom up to the envelope at x_min/x_max

    IMPORTANT: we do NOT draw an explicit full-length "contact" line at the
    valley level. That was the "first horizontal line" you wanted removed.
    The base thickness is still visible via the bottom line + end walls.
    """
    pts = poly_points_to_numpy(poly)
    if len(pts) == 0:
        return []
    ...
    # smooth the envelope a bit (reduces tessellation noise)
    ...
    # 2) base bottom line (shows thickness)
    polylines.append([(x_left, z_bottom), (x_right, z_bottom)])

    # 3) end walls from bottom up to the envelope
    polylines.append([(x_left, z_bottom), (x_left, float(z_top_s[0]))])
    polylines.append([(x_right, z_bottom), (x_right, float(z_top_s[-1]))])

    return polylines

def corrugation_levels_xz(poly: vtk.vtkPolyData, unit_scale, *, bins=1400, smooth_w=7):
    """Compute (z_contact, z_peak, z_bottom) in scaled units for end-view annotation."""
    pts = poly_points_to_numpy(poly)
    if len(pts) == 0:
        return None
    ...
    z_peak = float(np.max(z_top_s))
    z_bottom = float(np.min(z)) * unit_scale
    return z_contact, z_peak, z_bottom
def fit_polylines_to_box(
    polylines_2d,
    box_x,
    box_y,
    box_w,
...
def main():
    ...
    # Detect Z-levels from end walls for the End view (YZ)
    z_levels = detect_endwall_z_levels(poly, bounds, args.unit_scale)
    xmin, xmax, ymin, ymax, zmin, zmax = bounds
    ...
    for v in ("xy", "xz", "yz"):
        ...
        if v == "xy":
            ...
            polylines2d = outline_end_yz_clean(bounds, args.unit_scale, z_levels)
        else:
            polylines2d = outline_side_xz_clean(poly, bounds, args.unit_scale)

        # Keep drawings horizontally centered but top-aligned
        fitted, (x1, y1, x2, y2) = fit_polylines_to_box(
            polylines2d, bx, by, bw, bh, pad=pad, scale=global_scale, center_x=True, center_y=False
        )
