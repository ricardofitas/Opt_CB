...
def outline_end_yz_clean(bounds, unit_scale, levels):
    """End view (YZ) with key horizontal construction lines.

    We draw the outer bounding rectangle (ymin..ymax, z_bottom..z_peak) and add
    an internal horizontal line at z_contact (valley / base-top), matching what
    is visible in the side view.
    """
    xmin, xmax, ymin, ymax, zmin, zmax = bounds
    if levels is None:
        return outline_rect_yz(bounds, unit_scale)

    z_contact, z_peak, z_bottom = levels
    y0 = ymin * unit_scale
    y1 = ymax * unit_scale

    # Outer rectangle spanning the full visible Z range
    outer = [
        (y0, z_bottom),
        (y1, z_bottom),
        (y1, z_peak),
    ...
    # Internal line marking where the wave sits on the base
    inner = [(y0, z_contact), (y1, z_contact)]
...
def outline_side_xz_clean(poly: vtk.vtkPolyData, bounds, unit_scale,
    ...
    What we draw (no internal seam lines):
      ...
      3) End verticals that connect the base-top line to the envelope at x-min/x-max,
         so you see the lateral walls.
    ...
    # NOTE: We intentionally do NOT draw the bottom/base thickness line here.
    # For corrugator drawings you asked for a single base line under the wave.
...
def main():
    ...
    # Precompute corrugation levels for end-view annotation
    yz_levels = corrugation_levels_xz(poly, args.unit_scale)
    ...
    for v in ("xy", "xz", "yz"):
        ...
        if v == "xy":
            ...
            polylines2d = outline_end_yz_clean(bounds, args.unit_scale, yz_levels)
        else:
