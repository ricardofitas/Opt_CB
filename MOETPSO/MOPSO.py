...
def run_trial(trials):
    ...
    class Particle:
        def __init__(self, bounds): #, dim): AQUIIIIIIIIIIIIIIIIIIIII
            ...
            for i in range(len(bounds)):
                if i % 2 == 1:
                    self.position[i] = int(random.uniform(bounds[i][0], bounds[i][1]))
                else:
        ...
        if iiii:
            for i in range(len(position)):
                if i % 2 == 0:
                    ...
                    new_position[i] = int(max(bounds[i][0], min(bounds[i][1],position[i] + velocity[i])))
        else:
            for i in range(len(bounds)):
                if i % 2 == 1:
                    new_position[i] = int(random.uniform(bounds[i][0], bounds[i][1]))
                else:
    ...
    class PSO:
        def __init__(self, function, bounds, w, c1, c2, n_particles, n_iterations, funcoes_objetivo, folder): #AQUIIIIIIIIIIIIIIII
            ...
            for i in range(n_particles):
                ...
                objectives = z12(particle.position,dd, ll, Q)

                self.history_all.append([i, particle.position, particle.velocity, objectives, particle.fitness])
        ...
        def search(self):
            for i in range(self.n_iterations):
                ...
                pareto = [k[3] for k in self.history_all if k[5] == 1]
                conter_1 = 0
                for kk in pareto:
                ...
                for elem in self.history_all:
                    ...
                    if rank == 1 and objectives[2]:
                        ...
                        if objectives[1] < 0: print(elem)
                        X_all_values.append(X)

                plt.plot(xline_all_values, yline_all_values, 'o')

                save_iteration_data(self.folder, i, xline_all_values, yline_all_values, X_all_values, self.inc)
                ...
                for rank, objectives, cp in zip([k[5] for k in self.history_all],[k[3] for k in self.history_all], [k[8] for k in self.history_all]):
                    if rank < 5 and not rank == 1 and objectives[2]:
                        xline_all_values.append(objectives[0])
                        yline_all_values.append(objectives[1])
                ...
                for indk in range(len(self.history_all)):
                    self.history_all[indk] = self.history_all[indk][0:5]
                ...
                for k in range(n):          # CRIAÇÃO DAS NOVAS PARTICULAS
                    ...
                    objectives = z12(position,dd, ll, Q)

                    fitness = None

                    self.history_all.append([self.history_all[k][0], position, velocity, objectives, fitness])
    ...
    #-----------------------------RANK-----------------------------------------
        ...
        Nu = sum([len(k)==5 for k in history])
        ...
        for indk, k in enumerate(history):
            ...
            for rank_2,pair_2 in zip(list11,list2):
                if rank_2 == k[5] and pair_2 != tuple(point):
            ...
            #print(cda_1)
            history[indk].append(cda_1)


            #------------------RESTRICOES--------------------------------------

            history[indk].append(count)
            history[indk][4] = history[indk][6] + history[indk][7] - n_iterations * n_particles * history[indk][8]
    ...
    def z12(X, dd, ll, Q):
        ...
        for ii, elem in enumerate(X[1:]):
            if ii % 2 == 1:
                ...
                z2 += dist * abs(vh_load) * generate_speed_cost(velp)
    ...
    for problem_ind in range(1, 3):
        ...
        pso = PSO(function=score, bounds=bounds, n_particles=100, n_iterations=1000, w=0.7, c1=2.05, c2=2.05, funcoes_objetivo=objetivos, folder = "prob_GVRP/EPSO" + str(problem_ind) + "/EPSO_" + str(trials))
